...
/**
 * L’index des articles, page par page — `/blog/` puis `/blog/page/{n}/`.
 *
...
export default function ListeBlog({ page }: { page: number }) {
  const tous = articlesPublies();
  const pages = nombrePagesBlog();
  // Page 1 : l’article à la une et les 12 suivants. Page n : 12 cartes, sans « à la une ».
  const aLaUne = page === 1 ? tous[0] : undefined;
  const debut = page === 1 ? 1 : 1 + PAR_PAGE + (page - 2) * PAR_PAGE;
  const suivants = tous.slice(debut, debut + PAR_PAGE);
  const articles = tous;
      ...
      <main id="contenu">
        <HeroInterieur image={{ nom: "route-alpine", alt: "Mountain road to an Alpine resort" }}>
          <h1 className="max-w-3xl text-balance font-display text-titre-page">
            Ski transfer guides{page > 1 ? ` — page ${page}` : ""}
          </h1>
          ...
          <>
            ...
            {suivants.length > 0 ? (
              <Section fond="glacier">
                <EnTeteSection
                  surtitre={page === 1 ? "More" : `Page ${page} of ${pages}`}
                  titre={page === 1 ? "Other guides" : "Ski transfer guides"}
                />
                <ul
