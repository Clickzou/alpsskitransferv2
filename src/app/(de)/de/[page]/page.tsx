...
export async function generateMetadata({
  ...
  return metadataPage("de", page);
}

export default async function Page({ params }: { params: Promise<{ page: string }> }) {
  const { page } = await params;
  const contenu = pageIntlParSlug("de", page);
  if (!contenu) notFound();
  ...
  if (page === SLUGS_PREMIUM.de) {
    return (
      <PagePremium
        ...
        chapo={contenu.chapo}
        faq={contenu.faq}
        contenu={PAGES_PREMIUM.de}
        alternatives={alternativesPageIntl("de", contenu.equivalentEn)}
      />
    );
  }

  return <PageConversionIntl lang="de" page={contenu} />;
}
