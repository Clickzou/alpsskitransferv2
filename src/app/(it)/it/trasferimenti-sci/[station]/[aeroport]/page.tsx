...
export async function generateMetadata({
  ...
  return metadataTrajet("it", station, aeroport);
}
...
export default async function Trajet({
  ...
  const { station, aeroport } = await params;
  const donnees = resoudreTrajet("it", station, aeroport);
  if (!donnees) notFound();
  return <TrajetIntl lang="it" resort={donnees.resort} trajet={donnees.trajet} />;
}
