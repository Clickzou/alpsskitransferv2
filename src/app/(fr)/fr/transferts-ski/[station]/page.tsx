...
export async function generateMetadata({
  ...
  return metadataStation("fr", station);
}

export default async function Station({ params }: { params: Promise<{ station: string }> }) {
  const { station } = await params;

  const hub = hubPaysParSlug("fr", station);
  if (hub) return <HubPaysIntl lang="fr" hub={hub} />;

  const resort = resortParSlugTraduit("fr", station);
  if (!resort) notFound();
  return <StationIntl lang="fr" resort={resort} />;
}
