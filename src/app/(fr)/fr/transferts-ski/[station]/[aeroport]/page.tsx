...
export async function generateMetadata({
  ...
  return metadataTrajet("fr", station, aeroport);
}
...
export default async function Trajet({
  ...
  const { station, aeroport } = await params;
  const donnees = resoudreTrajet("fr", station, aeroport);
  if (!donnees) notFound();
  return <TrajetIntl lang="fr" resort={donnees.resort} trajet={donnees.trajet} />;
}
