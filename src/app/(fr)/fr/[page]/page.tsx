...
export async function generateMetadata({
  ...
  return metadataPage("fr", page);
}

export default async function Page({ params }: { params: Promise<{ page: string }> }) {
  const { page } = await params;
  const contenu = pageIntlParSlug("fr", page);
  if (!contenu) notFound();
  ...
  if (page === SLUGS_PREMIUM.fr) {
    return (
      <PagePremium
        ...
        chapo={contenu.chapo}
        faq={contenu.faq}
        contenu={PAGES_PREMIUM.fr}
        alternatives={alternativesPageIntl("fr", contenu.equivalentEn)}
      />
    );
  }

  return <PageConversionIntl lang="fr" page={contenu} />;
}
